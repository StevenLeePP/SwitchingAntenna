...
R15 最终裁决定死预注册 EVM²：中位倍率 2.112 超过 2× 门，ratio no-go；RMS-EVM
倍率 1.957 只作为凹压缩审计，不用于改判。三档 gain 实际是 33--43 dB 高 SNR 单簇，
所以 OTA 交付物限定为功能+趋势验证，并记录离线模型偏乐观约 2×，不得声称多 SNR
标定通过。Phase 2 已按该真实状态关闭；`phase2-freeze-2026-07-15` 仅表示实验资产
冻结，不表示所有门通过。
