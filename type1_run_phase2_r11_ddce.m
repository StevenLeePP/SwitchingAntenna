function report = type1_run_phase2_r11_ddce()
%TYPE1_RUN_PHASE2_R11_DDCE Preregistered decision-directed H update gate.
%   The R10 main point, Q=12 and three soft iterations are frozen.
p=type1_load_package(); [s0,s1]=r11_configs(); seeds=[20261001 20261003];
ber=zeros(2,5); evm=zeros(2,5); gap=zeros(2,2); rankFailures=zeros(2,1);
for k=1:2
    ...
    fixed=type1_analyze_ici_decision_feedback(l1.virtualRx30,p,12,3,'soft',true,true,'single','fixed');
    ddce=type1_analyze_ici_decision_feedback(l1.virtualRx30,p,12,3,'soft',true,true,'single','ddce');
    v3=type1_genie_replace_settling(l1.stitched122,l1.switchMeta.settlingBeta,l0.switchMeta.settlingBeta);
    r3=type1_analyze_user_cfo(v3,p,true); each={r0 r1 fixed ddce r3};
    for q=1:5
    ...
    rankFailures(k)=ddce.ddceRankDeficientCount;
    fprintf('seed=%d BER L0/L1/fixed/DDCE/L3=[%s] gap=[%s] rankFail=%d\n', ...
        seeds(k),num2str(ber(k,:),'%.5g '),num2str(gap(k,:),'%.3f '),rankFailures(k));
end
gapGain=gap(:,2)-gap(:,1); pass=all(ber(:,4)<ber(:,3)) && all(gapGain>=.15);
report=struct('seeds',seeds,'simL0',s0,'simL1',s1, ...
    'receiverNames',["L0" "L1" "fixedH" "DDCE" "L3"], ...
    'ber',ber,'evmPercent',evm,'gapClosure',gap,'gapGain',gapGain, ...
    'rankDeficientCount',rankFailures,'gapGainThreshold',.15,'ddcePassed',pass);
root=getenv('TYPE1_OFFLINE_OUTPUT_ROOT'); if isempty(root),root=tempdir;end
out=fullfile(root,['type1_phase2_r11_ddce_' char(datetime('now','Format','yyyyMMdd_HHmmss'))]);
mkdir(out); report.outputDir=out; save(fullfile(out,'phase2_r11_ddce.mat'),'report','-v7.3');
fprintf('DDCE pass=%d, result saved: %s\n',pass,out);
end
