...
矛盾，而是 $\Delta_{EVM^2}=(1+\Delta_{EVM})^2-1$ 的非线性口径差异。最终裁决定死
EVM² 并保留 ratio no-go：误差功率是自然线性域，RMS 的凹压缩会在 OTA 退化更大处
掩盖差异，不能事后用于宣告通过。

该实验还确定一个模型边界：改变 RX gain 没有在本次高 SNR 场景形成分离的 SNR 档，
且 matched-SNR 本身不能匹配真实 OTA baseline 中的信道估计误差/硬件残差组成。因此
R15 支持“高 SNR 单簇中损伤方向和整体量级可迁移”，并显示离线模型在该簇对新增
误差功率偏乐观约 2×；它不支持“只给定 SNR 即可精确预测任意 OTA 段的增量”，更
不支持“多 SNR 标定通过”。`cond(\hat H)` 仍只作估计信道诊断，不升级为真实信道
条件数。
