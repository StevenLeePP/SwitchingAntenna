...
assert(all(isfinite(r.estimatedConditionStats)),'type1:TDLA','TDL-A estimated condition statistics invalid.');
fprintf('Channel-model validation PASSED: TDL-A BER=[%s], cond(Hhat) p95=%.3f.\n',num2str(mean(r.infoBER,1),'%.3g '),r.estimatedConditionStats(2));
end
